...
class CmsGovCollector:
    ...
    def _collect(self, url: str, drpid: int) -> Dict[str, Any]:
        ...
        if not all_files:
            ...
            for r in all_files:
                ...
                planned.append({"name": name, "type": r.get("type", ""), "href": r.get("file_url", "")})
        ...
        # Determine data_types from slug data
        ...
        # Build files list for output
        files_list = self._build_files_list(slug_data, all_files, current_uuid, taxonomy_uuid)
        if files_list:
    ...
    def _build_files_list(
        ...
        """
        Build a list of file dicts with 'name' and 'href' keys.
        The href should be a relative path like:
        ?path=/datalumos/<id>/fcr:versions/V1/<filename>&type=file
        """
        files_out = []
        seen_names = set()

        # Try to get datalumos_id from taxonomy_uuid or current_uuid
        # The expected href format is: ?path=/datalumos/241282/fcr:versions/V1/filename.zip&type=file
        # We need to find the datalumos numeric ID

        datalumos_id = self._find_datalumos_id(slug_data, current_uuid, taxonomy_uuid)

        for r in all_files:
            file_url = r.get("file_url", "")
            raw_name = r.get("file_name") or file_url.split("/")[-1].split("?")[0]
            if not raw_name:
                continue
            name = sanitize_filename(raw_name) if raw_name else "dataset"
            ...
            # Build href
            href = self._build_file_href(file_url, raw_name, datalumos_id)
            files_out.append({"name": name, "href": href})

        return files_out

    def _find_datalumos_id(
        self,
        slug_data: Dict[str, Any],
        current_uuid: Optional[str],
        taxonomy_uuid: Optional[str],
    ) -> Optional[str]:
        """Try to extract the datalumos numeric ID from available data."""
        # Check slug_data for any numeric IDs
        for key in ["datalumos_id", "id", "numeric_id", "dataset_id"]:
        ...
        for key in ["datalumos_id", "id", "numeric_id"]:
            val = current.get(key)
            if val and str(val).isdigit():
                return str(val)

        # Try to extract from file URLs
        for key in ["file_url", "download_url"]:
            val = slug_data.get(key, "")
            if val:
                m = re.search(r'/datalumos/(\d+)/', val)
                if m:
                    return m.group(1)

        # Try fetching the dataset metadata to get the datalumos ID
        if current_uuid:
            try:
                endpoint = f"{_API_BASE}/dataset/{current_uuid}/resources"
                ...
                for r in resources:
                    file_url = r.get("file_url", "")
                    if file_url:
                        m = re.search(r'/datalumos/(\d+)/', file_url)
                        if m:
                            return m.group(1)
            except Exception:
                pass
    ...
    def _build_file_href(
        ...
        """
        Build a relative href for a file.
        Expected format: ?path=/datalumos/241282/fcr:versions/V1/filename.zip&type=file
        """
        if not file_url:
            return ""

        # Check if the URL already contains datalumos path info
        m = re.search(r'/datalumos/(\d+)/fcr:versions/(V\d+)/(.+?)(?:\?|$)', file_url)
        if m:
            dl_id = m.group(1)
            version = m.group(2)
            fname = m.group(3)
            return f"?path=/datalumos/{dl_id}/fcr:versions/{version}/{fname}&type=file"
        ...
        if "path=" in file_url:
            ...
            if path_val:
                file_type = qs.get("type", ["file"])[0]
                return f"?path={path_val}&type={file_type}"

        # Build from datalumos_id if available
        if datalumos_id and raw_name:
            safe_name = raw_name.replace(" ", "-").replace("_", "-")
            # Try to infer version from URL
            version = "V1"
            m2 = re.search(r'V(\d+)', file_url)
            if m2:
                version = f"V{m2.group(1)}"
            return f"?path=/datalumos/{datalumos_id}/fcr:versions/{version}/{safe_name}&type=file"

        # Return the file_url as-is if we can't build a proper href
        return file_url

    def _extract_geographic_coverage(self, slug_data: Dict[str, Any]) -> Optional[str]:
        """Extract geographic coverage from slug data."""
        # Check various fields for geographic info
        for key in ["geographic_coverage", "geography", "spatial_coverage", "geographic_scope"]:
            val = slug_data.get(key)
            if val:
                if isinstance(val, str):
                    ...
                    if isinstance(val[0], str):
                        ...
                        names = [v.get("name") or v.get("label") or "" for v in val if isinstance(v, dict)]
        ...
        # Check current_dataset
        current = slug_data.get("current_dataset") or {}
        for key in ["geographic_coverage", "geography", "spatial_coverage"]:
            val = current.get(key)
            if val:
                if isinstance(val, str):
                    return val
        ...
        # Check nav_topic for geographic hints
        nav_topic = slug_data.get("nav_topic") or {}
        if isinstance(nav_topic, dict):
            geo = nav_topic.get("geographic_coverage") or nav_topic.get("geography")
            if geo and isinstance(geo, str):
                return geo

        # Default for CMS data - it's typically US-wide
        # Only return "United States" if we have strong evidence this is a US dataset
        # Check if there are any US-specific indicators
        dataset_type = slug_data.get("dataset_type") or {}
        if isinstance(dataset_type, dict):
            geo = dataset_type.get("geographic_coverage")
            if geo:
                return geo
    ...
    def _determine_data_types(
        ...
        """
        Determine data_types from slug data.
        Expected values include combinations of:
        ...
        # Check slug_data for data_type info
        for key in ["data_type", "data_types", "dataset_type"]:
            val = slug_data.get(key)
            if val:
                if isinstance(val, str):
        ...
        for key in ["data_type", "data_types"]:
            val = current.get(key)
            if val:
                if isinstance(val, str) and val not in data_types:
                    data_types.append(val)
                elif isinstance(val, list):
                    for item in val:
                        if isinstance(item, str) and item not in data_types:
                            data_types.append(item)
