...
class CmsGovCollector:
    ...
    falls back to direct page scraping.
    """
    ...
    def _scrape_file_links(self, source_url: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Pattern: href contains datalumos path
            datalumos_patterns = [
    ...
    def _extract_files_from_json(self, data: Any) -> List[Dict[str, Any]]:
        ...
        files = []
        if isinstance(data, dict):
            # Check if this looks like a file resource
    ...
    def _extract_filename_from_href(self, href: str) -> str:
        ...
        if not href:
            return "dataset"
        # Try to get filename from path parameter
        ...
        try:
            ...
            for pattern in json_patterns:
                ...
                for json_str in matches:
                    try:
                        data = json.loads(json_str)
                        # Check if this looks like slug data
                        if isinstance(data, dict):
    ...
    def _scrape_description(self, url: str, drpid: int) -> Optional[str]:
        ...
        if not self._ensure_browser():
            ...
            return None
        try:
            # Check if page is already loaded (we may have loaded it during scrape_page_metadata)
            try:
