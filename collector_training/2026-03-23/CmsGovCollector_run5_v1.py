...
# DataLumos base URL for file hrefs
...
class CmsGovCollector:
    ...
    def _collect(self, url: str, drpid: int) -> Dict[str, Any]:
        ...
        # Get datalumos_id from storage record for constructing file hrefs
        storage_record = Storage.get(drpid)
        datalumos_id = storage_record.get("datalumos_id") if storage_record else None
        ...
        if not all_files:
            ...
            for r in all_files:
                ...
                file_entry = {"name": name, "type": r.get("type", "")}
                # Build href if we have datalumos_id
                href = self._build_file_href(r, datalumos_id, name)
                if href:
                    file_entry["href"] = href
                planned.append(file_entry)
        ...
        # Determine data_types - check if dataset has clinical data too
    ...
    def _build_file_href(
        ...
        """
        Build a DataLumos-style href for a file resource.
        Format: ?path=/datalumos/<id>/fcr:versions/V1/<sanitized-filename>&type=file
        """
        if not datalumos_id:
            return None

        # Try to get the file URL and construct a DataLumos path
        file_url = resource.get("file_url", "")
        if not file_url:
            return None

        # Sanitize filename for URL use (replace spaces with dashes, etc.)
        url_filename = filename.replace(" ", "-").replace("_", "-")
        # Remove consecutive dashes
        url_filename = re.sub(r'-+', '-', url_filename)

        href = f"?path=/datalumos/{datalumos_id}/fcr:versions/V1/{url_filename}&type=file"
        return href
    ...
    def _determine_data_types(
        ...
        """
        Determine data types for the dataset.
        Most CMS datasets are administrative records data.
        Some also include clinical data.
        """
        data_types = [_CMS_DATA_TYPES]

        # Check dataset description/name/tags for clinical indicators
        ...
        for tag in tags:
            if isinstance(tag, str):
                ...
                tag_names.append((tag.get("name") or "").lower())

        clinical_keywords = ["clinical", "diagnosis", "procedure", "icd", "cpt", "hcpcs", "medical"]
        all_text = name + " " + " ".join(tag_names)

        if any(kw in all_text for kw in clinical_keywords):
            if "clinical data" not in data_types:
                data_types.append("clinical data")

        # Check the nav_topic for clinical indicators
    ...
    def _extract_geographic_coverage(self, slug_data: Dict[str, Any]) -> Optional[str]:
        ...
        # Check for geographic_coverage field
        geo = slug_data.get("geographic_coverage") or slug_data.get("geography") or ""
        if geo and isinstance(geo, str):
            return geo.strip()

        # Check current_dataset for geo info
        current = slug_data.get("current_dataset") or {}
        geo = current.get("geographic_coverage") or current.get("geography") or ""
        if geo and isinstance(geo, str):
            return geo.strip()

        # Most CMS datasets cover the United States
        # Check if there's any geographic scope indicator
        spatial = slug_data.get("spatial") or slug_data.get("spatial_coverage") or ""
        if spatial and isinstance(spatial, str):
            return spatial.strip()

        # Default for CMS national datasets
        name = (slug_data.get("name") or "").lower()
        nav_topic = slug_data.get("nav_topic") or {}
        topic_name = ""
        if isinstance(nav_topic, dict):
        ...
        # CMS data is typically national US data
    ...
    def _list_output_files(
        ...
        """
        Build the 'files' metadata list from gathered file resources.
        Each entry: {name, href} where href follows the DataLumos pattern.
        """
        output = []
        for resource in all_files:
            ...
            if datalumos_id:
                href = self._build_file_href(resource, datalumos_id, name)
                if href:
                    entry["href"] = href
    ...
    def _format_date(self, date_str: str) -> str:
        """
        Format date string - return year if only year is known,
        or M/D/YYYY format for full dates.
        """
        if not date_str:
            return date_str
