...
Key insight about worst-scoring projects:
  - URLs like /cms-innovation-center-programs/... where the page returns a 404
    or has no real data should return NO metadata at all (all fields None)
  - The collector must detect these "dead" pages and return empty result
  - For pages WITH data but no slug API, we scrape and extract file links
"""
...
class CmsGovCollector:
    ...
    def _collect(self, url: str, drpid: int) -> Dict[str, Any]:
        ...
        if not slug_data and is_innovation:
            ...
            # Navigate to the page to check it
            page_loaded = self._load_page(url)
            if not page_loaded:
                Logger.info("Could not load innovation center page, returning empty: %s", url)
                return {}
        ...
        if slug_data:
            ...
            # Build files list for storage (href-based references)
            files_list = self._build_files_list(all_files, current_uuid, taxonomy_uuid)
            if files_list:
    ...
    def _innovation_page_has_data(self, url: str, drpid: int) -> bool:
        ...
        Key insight: pages that are 404s or "coming soon" pages should return False.
        Pages with actual dataset content (download links, data tables, etc.) return True.
        """
        if not self._ensure_browser():
            return False
        ...
        try:
            ...
            # Page should already be loaded from _load_page call
            ...
            # Only check title/h1 for 404-like content, not full body
            try:
            ...
            # Check for resource/download sections
            resource_patterns = [
                r'"resources":\s*\[',
                r'"file_url":\s*"[^"]+',
                r'data-resource',
            ...
            for pattern in resource_patterns:
                if re.search(pattern, page_content, re.IGNORECASE):
                    Logger.info("Found resource pattern '%s' on page: %s", pattern, url)
                    return True
    ...
    def _scrape_file_links(self, source_url: str) -> List[Dict[str, Any]]:
        ...
        Specifically handles the ?path= pattern used by DataLumos.
        """
        files = []
        try:
            page_content = self._page.content()
            ...
            # Pattern: href="?path=/datalumos/NNN/fcr:versions/V1/filename.zip&type=file"
            datalumos_patterns = [
    ...
    def _build_files_list(
        ...
        """
        Build a files list with name and href for storage.
        The href uses the DataLumos ?path= pattern when available,
        or the direct file_url from the API.
        """
        files_list = []
        seen = set()
        ...
        for resource in all_files:
            ...
            # Use the file_url directly as the href
            files_list.append({
                "name": name,
                "href": file_url,
            })
    ...
    def _update_storage(self, drpid: int, result: Dict[str, Any]) -> None:
        ...
        # we should NOT update storage with empty values - leave as sourced
        # so it can be re-evaluated or skipped
        if not result:
