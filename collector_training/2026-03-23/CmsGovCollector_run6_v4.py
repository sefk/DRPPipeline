...
class CmsGovCollector:
    ...
    def _collect(self, url: str, drpid: int) -> Dict[str, Any]:
        ...
        if not all_files:
            ...
            # Format matches _list_output_files: [{name, href, ...}]
            planned = []
            for r in all_files:
                raw_name = r.get("file_name") or r.get("file_url", "").split("/")[-1].split("?")[0]
                name = sanitize_filename(raw_name) if raw_name else "dataset"
                file_url = r.get("file_url", "")
                # Build href that matches the expected format
                href = self._build_file_href(r, slug_data)
                planned.append({
                    "name": name,
                    "href": href,
                    "type": r.get("type", ""),
        ...
        # Determine data_types - may be more specific than just administrative records
        ...
        result["_files_metadata"] = all_files

        return result

    def _build_file_href(self, resource: Dict[str, Any], slug_data: Dict[str, Any]) -> str:
        """
        Build the href for a file resource that matches the expected format.
        ...
        # If the file_url already has the datalumos path format, use it
        if "datalumos" in file_url:
            return file_url

        # Try to construct from available data
        # The expected href format appears to be a query parameter path
        if file_url:
            # Parse the file URL to extract useful components
            parsed = urlparse(file_url)
            path = parsed.path

            # Check if there's a datalumos-style path in the URL
            if "/datalumos/" in path:
                # Extract the relevant portion
                datalumos_idx = path.index("/datalumos/")
                return f"?path={quote(path[datalumos_idx:])}&type=file"
    ...
    def _determine_data_types(
        ...
        """
        Determine data types for this dataset.
        CMS datasets are primarily administrative records data.
        Some may also include clinical data.
        """
        # Start with base data type
        data_types = [_CMS_DATA_TYPES]

        # Check if this dataset also contains clinical data
        # by looking at dataset type, tags, or title
        name = slug_data.get("name") or ""
        tags = []
        for tag in (slug_data.get("tags") or []):
        ...
        clinical_keywords = [
            ...
            "medical", "health condition", "disease", "treatment",
            "joint replacement", "hospital", "inpatient", "outpatient",
            "beneficiar", "patient", "care", "quality measure"
        ]

        name_lower = name.lower()
        is_clinical = any(kw in name_lower for kw in clinical_keywords)
        is_clinical = is_clinical or any(kw in dt_name for kw in clinical_keywords)
        is_clinical = is_clinical or any(
            any(kw in tag for kw in clinical_keywords) for tag in tags
        )

        if is_clinical and "clinical data" not in data_types:
    ...
    def _extract_geographic_coverage(self, slug_data: Dict[str, Any]) -> Optional[str]:
        ...
        # Check for geographic coverage fields
        geo = slug_data.get("geographic_coverage") or slug_data.get("geography") or ""
        if isinstance(geo, str) and geo.strip():
            return geo.strip()
        elif isinstance(geo, dict):
            geo_name = geo.get("name") or geo.get("label") or ""
            if geo_name:
                return geo_name

        # Check for spatial/location fields
        spatial = slug_data.get("spatial") or slug_data.get("location") or ""
        ...
        for key in ["geographic_coverage", "geography", "spatial", "location"]:
            val = current.get(key)
            if val:
        ...
        # Default: CMS data is typically United States coverage
        # Check if there's any state-level indicator
        name = slug_data.get("name") or ""
        nav_topic = slug_data.get("nav_topic") or {}
        if isinstance(nav_topic, dict):
            nav_name = nav_topic.get("name") or ""
        else:
            nav_name = ""

        # Most CMS national datasets cover the United States
    ...
    def _extract_date_range_from_metadata(
        ...
        4. Extract year from file names
        """
                ...
                for key in ["data_start_date", "start_date", "coverage_start"]:
                    ...
                    if val:
                        ...
                        for end_key in ["data_end_date", "end_date", "coverage_end"]:
                            ...
                            if end_val:
                                result["time_end"] = self._format_date(str(end