...
# Standard collection notes template
_COLLECTION_NOTES_DATE = date.today().strftime("%-m/%-d/%Y")
...
class CmsGovCollector:
    ...
    def _collect(self, url: str, drpid: int) -> Dict[str, Any]:
        ...
        # Extract time_start / time_end from dataset_version_date on Primary resources.
        # Then try to get better dates from the slug/metadata API.
        date_range = self._extract_date_range_from_metadata(slug_data, all_files)
        ...
        # Set collection notes with download date
        today = date.today()
    ...
    def _extract_keywords(self, slug_data: Dict[str, Any]) -> str:
        ...
        Try tags, themes, nav_topic, and other available fields.
        """
        keywords = []
        ...
        # Try 'nav_topic' for category info
    ...
    def _extract_date_range_from_metadata(
        ...
        2. dataset version temporal metadata
    ...
    def _format_date(self, date_str: str) -> str:
        """Return date_str as-is (keep original format for now)."""
        return date_str
    ...
    def _scrape_metadata_from_page(self, url: str, drpid: int) -> Dict[str, Any]:
        """
        Scrape additional metadata from the rendered page including:
        - Description/summary
        - Keywords/tags
        - Temporal coverage
        - Agency info
        ...
        try:
            # Try to get temporal coverage from the page
            for selector in [
                "[class*='temporal']",
                "[class*='date-range']",
                "[class*='coverage']",
                ...
                if el:
                    text = el.inner_text().strip()
                    if text:
                        Logger.info("Found temporal element: %s", text[:100])
                        break

            # Try to extract keywords from the page
            for selector in [
                "[class*='tag']",
                "[class*='keyword']",
                "[class*='topic']",
                ...
                if els:
                    kws = [el.inner_text().strip() for el in els if el.inner_text().strip()]
                    if kws:
                        result["page_keywords"] = kws
                        break
        except Exception as exc:
            Logger.warning("Error scraping metadata from page: %s", exc)
